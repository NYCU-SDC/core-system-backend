...
func (q *Queries) ListByIDs(ctx context.Context, dollar_1 []uuid.UUID) ([]ListByIDsRow, error) {
	rows, err := q.db.Query(ctx, listByIDs, dollar_1)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListByIDsRow
	for rows.Next() {
		var i ListByIDsRow
		if err := rows.Scan(
			&i.ID,
